document.addEventListener('DOMContentLoaded', () => {
    ...
    if(contactForm){
        contactForm.addEventListener('submit', async (e) => {
            ...
            const formData = {
                childName: contactForm.parentName.value.trim(),
                ageGroup: contactForm.email.value.trim(),
                dietaryRestrictions: contactForm.phone.value.trim(),
                preferredMeals: contactForm.additionalInfo.value.trim(),
                recaptcha: recaptchaResponse
            };

            try {
                const response = await fetch('submit_contact.php', { // Update with your backend endpoint
                    method: 'POST',
                    headers: {
    ...
    if(dietaryForm){
        dietaryForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('submit_dietary_needs.php', { // Update with your backend endpoint
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
    ...
    if(tourForm){
        tourForm.addEventListener('submit', async (e) => {
            ...
            const formData = {
                parentName: tourForm.tourParentName.value.trim(),
                email: tourForm.tourEmail.value.trim(),
            ...
            try {
                const response = await fetch('schedule_tour.php', { // Update with your backend endpoint
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                if (response.ok) {
                    alert(currentLanguage === 'en' ? 'Your tour has been scheduled!' : '¡Su visita ha sido programada!');
                    tourForm.reset();
                    grecaptcha.reset();
                    tourForm.classList.remove('was-validated');
                } else {
                    alert(currentLanguage === 'en' ? 'There was an error scheduling your tour. Please try again later.' : 'Hubo un error al programar su visita. Por favor, inténtelo de nuevo más tarde.');
                }
            } catch (error) {
                console.error('Error:', error);
                alert(currentLanguage === 'en' ? 'There was an error scheduling your tour. Please try again later.' : 'Hubo un error al programar su visita. Por favor, inténtelo de nuevo más tarde.');
